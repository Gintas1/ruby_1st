...
describe Admin do
  describe '#initialize' do
    ...
    it 'checks if gamelist is created' do
      admin = Admin.new(username: 'test', password: 'test')
      expect(admin.gamelist).to be_empty
    end
    it 'checks if purchase list is created' do
      admin = Admin.new(username: 'test', password: 'test')
      expect(admin.purchases).to be_empty
    end
  ...
  describe 'adds' do
    ...
    it 'new admin' do
      admin = Admin.new(username: 'test', password: 'test',
                        admin_granted: 'test')
      new_admin = admin.add_admin(username: 'test', password: 'test',
                                  admin_granted: admin)
  ...
  describe 'edits' do
    ...
    it 'game description' do
      ...
      expect { admin.edit_game_description(game, 'test') }
              .to change { game.description }.from('Game description test')
                                             .to('test')
    end
    it 'comment' do
      admin = Admin.new(username: 'test', password: 'test',
      ...
      comment = Comment.new(text: 'text test',
                            ...
                            id: 1)
      expect { admin.edit_comment(comment, 'text') }
              .to change { comment.text }.from('text test').to('text')
    end
    it 'user balance' do
      admin = Admin.new(username: 'test', password: 'test',
                        admin_granted: 'test')
      user = User.new(username: 'test', password: 'test',
                      admin_granted: 'test')
      expect { admin.edit_user_balance(user, 10) }
              .to change { user.balance }.from(0).to(10)
    end
  end
  describe 'removes' do
    it 'game from shop' do
      ...
      game = Game.new(name: 'Game name test', genre: 'Genre test',
                               description: 'Game description test', price: 10)
      gamelist.push(game)
      admin.remove_game(game, gamelist)
      expect(gamelist).not_to include(game)
    end
    it 'game that does not exist' do
      ...
      game = Game.new(name: 'Game name test', genre: 'Genre test',
                               description: 'Game description test', price: 10)
      expect { admin.remove_game(game, gamelist) }.to output.to_stdout
    end
  end
  describe 'blocks' do
    it 'user' do
      admin = Admin.new(username: 'test', password: 'test',
                        admin_granted: 'test')
      user = User.new(username: 'test', password: 'test')
      expect { admin.block_user(user) }
              .to change { user.blocked }.from(false).to(true)
    end
    it 'unblocks user' do
      admin = Admin.new(username: 'test', password: 'test',
                        admin_granted: 'test')
      user = User.new(username: 'test', password: 'test')
      user.blocked = true
      expect { admin.unblock_user(user) }
              .to change { user.blocked }.from(true).to(false)
    end
  end
  describe 'gets' do
